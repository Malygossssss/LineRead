...
LineRead 使用 Playwright 控制微信读书公开网页版。普通章节直接读取已经渲染的 DOM；正式正文以 Canvas 显示时，通过页面自带控件切换到纵向布局，对完整章节 Canvas 截图运行本地 RapidOCR。它不读取浏览器缓存、不拦截私有接口，也不尝试解密微信读书数据，截图和正文不会上传到 OCR 服务。
